...
function createSkyEffects() {
  ...
  for (let i = 0; i < 46; i += 1) {
    ...
    if (isStoryStar) {
      sparkle.style.setProperty("--star-size", `${5 + Math.random() * 6}px`);
    }
...
async function saveToSheet(payload) {
  if (
    !CONFIG.apiEndpoint ||
    CONFIG.apiEndpoint.includes("PEGA_AQUI")
  ) {
    return false;
  }
...
function buildScannerUrl(code) {
  // El QR abre una página de GitHub Pages, no la interfaz de Apps Script.
  // Así se evita el aviso de Google y los bloqueos de HTMLService.
  const url = new URL(
    CONFIG.scannerPage,
...
function applyPersonalizedGuest(guest) {
  ...
  const name =
    guest.invitado || "Invitación personalizada";
  ...
  $("#passCapacity").textContent =
    `${guest.cupoAsignado} persona` +
    `${guest.cupoAsignado === 1 ? "" : "s"}`;

  $("#passConfirmation").textContent =
    guest.confirmacion || "Pendiente";

  $("#passRemaining").textContent =
    String(guest.cupoRestante);
...
function setupRsvp() {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    const text = [
      ...
      "",
      `*Invitación:* ${
        personalizedGuest?.invitado || payload.nombre
      }`,
      invitationCode
