...
async function saveToSheet(payload) {
  if (!CONFIG.apiEndpoint || CONFIG.apiEndpoint.includes("PEGA_AQUI")) return false;
...
function getInvitationCodeFromUrl() {
  ...
  return normalizeInvitationCode(params.get("codigo") || params.get("code"));
}

function requestJsonp(params) {
  return new Promise((resolve, reject) => {
    if (!CONFIG.apiEndpoint || CONFIG.apiEndpoint.includes("PEGA_AQUI")) {
      reject(new Error("Falta configurar la URL de Google Apps Script."));
      return;
    }

    const callbackName = `babyPauloCallback_${Date.now()}_${Math.random().toString(36).slice(2)}`;
    ...
    script.onerror = () => {
      cleanup();
      reject(new Error("No fue posible conectar con la lista de invitados."));
    };
...
function renderGuestQr(code) {
  const qrContainer = $("#guestQr");
  if (!qrContainer) return;
  qrContainer.innerHTML = "";

  if (typeof QRCode !== "function") {
    qrContainer.textContent = "No se pudo cargar el generador de QR.";
    return;
  }

  new QRCode(qrContainer, {
    text: buildScannerUrl(code),
    width: 230,
    height: 230,
    colorDark: "#263943",
    colorLight: "#ffffff",
...
function applyPersonalizedGuest(guest) {
  ...
  const name = guest.invitado || "Invitación personalizada";
  const introGuest = $("#introGuest");
  const guestNameInput = $("#guestName");
  const guestNameHelp = $("#guestNameHelp");

  if (introGuest) {
    introGuest.textContent = name;
    introGuest.hidden = false;
  }
  ...
  if (guestNameHelp) {
    guestNameHelp.textContent = "Nombre asociado al código único de esta invitación.";
  }

  $("#passGuestName").textContent = `Pase de ${name}`;
  $("#passInvitationName").textContent = name;
  $("#passCapacity").textContent = `${guest.cupoAsignado} persona${guest.cupoAsignado === 1 ? "" : "s"}`;
  $("#passConfirmation").textContent = guest.confirmacion || "Pendiente";
  $("#passRemaining").textContent = String(guest.cupoRestante);
  $("#passCode").textContent = guest.codigo;

  $("#passSummary").hidden = false;
  $("#guestPassQrArea").hidden = false;
  $("#passLoading").hidden = true;
  renderGuestQr(guest.codigo);
}

async function setupPersonalizedInvitation() {
  const section = $("#pasePersonalizado");
  if (!section) return;

  invitationCode = getInvitationCodeFromUrl();
  if (!invitationCode) return;

  section.hidden = false;

  try {
    const response = await requestJsonp({ action: "guest", codigo: invitationCode });
    if (!response?.ok || !response.guest) {
      throw new Error(response?.message || "El código de invitación no existe o ya no está activo.");
    }
    applyPersonalizedGuest(response.guest);
  } catch (error) {
    $("#passLoading").hidden = true;
    const errorBox = $("#passError");
...
function setupRsvp() {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    const text = [
      ...
      invitationCode ? `*Código:* ${invitationCode}` : "",
      personalizedGuest ? `*Cupo asignado:* ${personalizedGuest.cupoAsignado}` : "",
    ...
    status.textContent = CONFIG.apiEndpoint && !CONFIG.apiEndpoint.includes("PEGA_AQUI")
